...
def validate_manifest(value: object, root: Path) -> dict[str, object]:
    ...
    if release != "3.7.2":
        fail("Release 3.7 manifest must identify version 3.7.2")
    skill = _fields(
        manifest["skill"],
    ...
    for profile_index, profile_value in enumerate(profiles):
        ...
        if contract is None or (
            ...
            fail(f"Profile {profile_id} identity and roots do not match Release 3.7")
    ...
    if baseline != {"scope": "project", "mode": "copy"}:
        fail("Release 3.7 live baseline must be project copy")
