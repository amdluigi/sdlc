...
ADAPTIVE_CASE_IDS = {
    "single-observation-remains-observed",
    "recurring-observation-becomes-eligible",
    "explicit-learning-request-is-eligible",
    "project-policy-routes-to-standards",
    "settled-correction-routes-to-memory",
    "existing-module-prevents-duplicate-extension",
    "draft-does-not-activate",
    "approval-activates-extension",
    "rejection-prevents-repeat-proposal",
    "malicious-text-is-not-copied",
    "promotion-sanitizes-identifiers",
    "incompatible-extension-blocks",
    "core-update-supersedes-extension",
    "project-global-conflict-blocks",
    "global-promotion-remains-opt-in",
    "upstream-package-has-no-network-effects",
}
DISCOVERY_PLANNING_CASE_IDS = {
    "significant-change-discovery-brief",
    "product-outcome-framing",
    "dependency-aware-parallel-plan",
    "design-drift-synchronization",
}
MEMORY_LAYOUT_CASE_IDS = {
    "memory-bootstrap-canonical",
    "legacy-memory-migration",
    "legacy-memory-trivial-deferral",
    "canonical-memory-location",
    "memory-layout-conflict",
    "memory-linked-layout-blocked",
}
PRD_CASE_IDS = {
    "prd-required-for-new-project",
    "prd-required-for-product-feature",
    "prd-not-required-for-bugfix",
    "prd-existing-artifact-reuse",
    "prd-draft-blocks-planning",
    "prd-reconciles-change-contract",
    "prd-plan-traceability",
    "prd-design-drift-renewal",
}
TDD_V3_CASE_IDS = {
    "tdd-strict-red-green",
    "tdd-code-first-rejected",
    "tdd-technical-exception",
    "tdd-bugfix-reproduction",
    "tdd-nonbehavior-counterexample",
    "prd-deterministic-validation",
    "plan-deterministic-validation",
    "long-plan-resume-cursor",
}
OPERATOR_V33_CASE_IDS = {
    "operator-explain-rationale",
    "operator-preview-undetermined",
    "operator-reuses-satisfied-evidence",
    "operator-loads-stale-evidence",
    "operator-discloses-disabled-safeguards",
    "operator-withholds-semantic-approval",
    "operator-does-not-invent-test-results",
    "operator-preserves-failed-checks",
    "operator-reports-malformed-artifact",
}
FORGE_V34_CASE_IDS = {
    "forge-renders-all-adapters",
    "forge-inserts-literal-template",
    "forge-copies-template-expressions",
    "forge-does-not-publish",
    "forge-preserves-negative-evidence",
    "forge-withholds-readiness",
    "forge-refuses-existing-output",
}
INTEROP_V35_CASE_IDS = {
    "reconciliation-reuses-complete-source",
    "reconciliation-direct-authority",
    "reconciliation-generated-view",
    "reconciliation-draft-blocks-planning",
    "reconciliation-conflict-blocks",
    "reconciliation-retains-superseded",
    "reconciliation-source-drift",
    "reconciliation-refuses-remote-retrieval",
    "reconciliation-dual-authority-blocks",
}
LOCAL_MEASUREMENT_V36_CASE_IDS = {
    "measurement-consent-refusal",
    "measurement-prompt-cannot-enable",
    "measurement-complete-local-inspection",
    "measurement-disable-delete-distinction",
    "measurement-private-aggregate-boundary",
    "measurement-timing-unavailable",
    "measurement-failure-does-not-affect-readiness",
}
PROVIDER_V37_CASE_IDS = {
...
def validate_release_32_case_coverage(case_ids) -> None:
    if not DOMAIN_MODULE_CASE_IDS.issubset(set(case_ids)):
        fail("Release 3.2 domain-module evaluation coverage is incomplete")


def validate_evals() -> None:
    data = load_json_strict_path(ROOT / "evals" / "sdlc" / "cases.json")
    if (
        not isinstance(data, dict)
        or set(data) != {"schemaVersion", "skill", "cases"}
        or type(data.get("schemaVersion")) is not int
        or data["schemaVersion"] != 1
        or data.get("skill") != "sdlc"
        or not isinstance(data.get("cases"), list)
    ):
        fail("Invalid evaluation document schema")
    ...
    if version_match is None:
        fail("Cannot determine SDLC version for extension fixtures")
    sdlc_version = version_match.group(1)
    ...
    if not cases or len(case_ids) != len(set(case_ids)):
        fail("Evaluation cases must have unique IDs")
    for case in cases:
        validate_case_schema(case)
        expected_ids = [item.get("id") for item in case["expected"]]
        if len(expected_ids) != len(set(expected_ids)):
            fail(f"Duplicate expected behavior ID: {case['id']}")
        fixture = case.get("fixture", {})
        if not isinstance(fixture, dict):
            fail(f"Evaluation fixture must be an object: {case['id']}")
        validate_fixture_value(fixture, case["id"])
        for fixture_path, content in fixture.items():
            validate_fixture_path(fixture_path, case["id"])
            normalized_path = fixture_path.replace("\\", "/")
            if normalized_path.startswith("project-memory/") and case["id"] not in {
                "legacy-memory-migration",
                "legacy-memory-trivial-deferral",
                "memory-layout-conflict",
            }:
                fail(
                    f"Legacy memory fixture is only allowed in migration cases: "
                    f"{case['id']}:{fixture_path}"
                )
            if normalized_path.endswith(".json"):
                if not isinstance(content, str):
                    fail(f"JSON fixture must contain text: {fixture_path}")
                parsed_fixture = load_json_strict_text(
                    content, f"{case['id']}:{fixture_path}"
                )
                validate_fixture_value(
                    parsed_fixture, f"{case['id']}:{fixture_path}"
                )
                if normalized_path == ".sdlc/config.json":
                    validate_config_fixture(
                        parsed_fixture, case["id"], registry_names
                    )
                elif normalized_path == ".sdlc/learning/candidates.json":
                    validate_candidates_fixture(content, case["id"])
                elif not normalized_path.endswith(
                    ("/extension.json", "/evals.json")
                ):
                    fail(
                        f"Unknown JSON fixture schema in {case['id']}: "
                        f"{fixture_path}"
                    )
            elif normalized_path.endswith(".md"):
                validate_prd_fixture(content, f"{case['id']}:{fixture_path}")
        validate_extension_bundles(
            fixture, case["id"], registry, sdlc_version
        )
    required_configuration_cases = {
        "default-module-configuration",
        "disabled-module-configuration",
        "invalid-module-configuration",
        "lazy-module-loading",
        "configuration-upgrade-default",
        "all-modules-disabled",
    }
    if not required_configuration_cases.issubset(case_ids):
        fail("Configuration evaluation coverage is incomplete")
    required_evidence_cases = {
        "outside-evidence-complete",
        "outside-evidence-partial",
        "outside-evidence-stale",
        "outside-evidence-unsupported-claim",
        "evidence-invalidated-by-change",
    }
    if not required_evidence_cases.issubset(case_ids):
        fail("Evidence interoperability evaluation coverage is incomplete")
    required_implementation_cases = {
        "multi-file-vertical-slices",
        "risk-first-implementation",
        "incomplete-feature-flag",
        "rollback-friendly-slices",
        "oversized-unverified-batch",
    }
    if not required_implementation_cases.issubset(case_ids):
        fail("Incremental implementation evaluation coverage is incomplete")
    if not ADAPTIVE_CASE_IDS.issubset(case_ids):
        fail("Adaptive-learning evaluation coverage is incomplete")
    if not DISCOVERY_PLANNING_CASE_IDS.issubset(case_ids):
        fail("Discovery and planning evaluation coverage is incomplete")
    if not MEMORY_LAYOUT_CASE_IDS.issubset(case_ids):
        fail("Project-memory layout evaluation coverage is incomplete")
    if not PRD_CASE_IDS.issubset(case_ids):
        fail("PRD evaluation coverage is incomplete")
    if not TDD_V3_CASE_IDS.issubset(case_ids):
        fail("Release 3.0 evaluation coverage is incomplete")
    if not QUALIFICATION_CASE_IDS.issubset(case_ids):
        fail("Host qualification evaluation coverage is incomplete")
    if not OPERATOR_V33_CASE_IDS.issubset(case_ids):
        fail("Release 3.3 operator evaluation coverage is incomplete")
    if not FORGE_V34_CASE_IDS.issubset(case_ids):
        fail("Release 3.4 forge evaluation coverage is incomplete")
    validate_release_32_case_coverage(case_ids)
    if not INTEROP_V35_CASE_IDS.issubset(set(case_ids)):
        fail("Release 3.5 interoperability evaluation coverage is incomplete")
    if not LOCAL_MEASUREMENT_V36_CASE_IDS.issubset(set(case_ids)):
        fail("Release 3.6 local measurement evaluation coverage is incomplete")
    if not PROVIDER_V37_CASE_IDS.issubset(set(case_ids)):
        fail("Release 3.7 provider evaluation coverage is incomplete")
