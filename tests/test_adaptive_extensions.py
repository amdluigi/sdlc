...
class BehavioralFixtureValidationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.cases_path = (
        ...
        cls.document = json.loads(cls.cases_path.read_text(encoding="utf-8"))

    def _validate(self, document):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            path = root / "evals" / "sdlc" / "cases.json"
            path.parent.mkdir(parents=True)
            path.write_text(json.dumps(document), encoding="utf-8")
            with mock.patch.object(repository_validate, "ROOT", root):
                repository_validate.validate_evals()

    def _case(self, document, case_id):
        return next(
            case for case in document["cases"] if case["id"] == case_id
        )

    def test_current_behavioral_fixtures_are_helper_valid(self):
        self._validate(copy.deepcopy(self.document))

    def test_every_json_fixture_is_parsed_strictly(self):
        document = copy.deepcopy(self.document)
        case = self._case(document, "single-observation-remains-observed")
        case["fixture"][".sdlc/learning/candidates.json"] = (
    ...
    def test_json_fixture_schemas_are_checked_by_role(self):
        mutations = (
            (
                "single-observation-remains-observed",
                ".sdlc/learning/candidates.json",
            ...
            (
                "draft-does-not-activate",
                ".sdlc/extensions/verify-generated-output/extension.json",
                '{"schemaVersion":1}',
                "extension metadata",
            ),
            (
                "draft-does-not-activate",
                ".sdlc/extensions/verify-generated-output/evals.json",
                '{"schemaVersion":2,"cases":[]}',
                "evals.json",
            ),
        )
        for case_id, fixture_path, content, message in mutations:
            with self.subTest(fixture_path=fixture_path):
                document = copy.deepcopy(self.document)
                self._case(document, case_id)["fixture"][fixture_path] = content
                with self.assertRaisesRegex(ValueError, message):
                    self._validate(document)

    def test_schema_two_activation_maps_require_booleans(self):
        document = copy.deepcopy(self.document)
        case = self._case(document, "approval-activates-extension")
        case["fixture"][".sdlc/config.json"] = (
            '{"schemaVersion":2,"modules":{"continuous-improvement":true},'
            '"extensions":{"project":{"verify-generated-output":"1.0.0"},'
            '"global":{}}}'
        )
    ...
    def test_extension_bundle_requires_exact_helper_files(self):
        document = copy.deepcopy(self.document)
        case = self._case(document, "draft-does-not-activate")
        del case["fixture"][
            ".sdlc/extensions/verify-generated-output/evals.json"
        ]
    ...
    def test_extension_module_path_must_match_bundle_module(self):
        document = copy.deepcopy(self.document)
        case = self._case(document, "draft-does-not-activate")
        metadata_path = (
            ".sdlc/extensions/verify-generated-output/extension.json"
        )
        metadata = json.loads(case["fixture"][metadata_path])
        metadata["path"] = "nested/MODULE.md"
        case["fixture"][metadata_path] = json.dumps(metadata)

        with self.assertRaisesRegex(ValueError, "identify MODULE.md"):
            self._validate(document)

    def test_global_catalog_layout_is_checked(self):
        document = copy.deepcopy(self.document)
        case = self._case(document, "project-global-conflict-blocks")
        case["fixture"][
            "global-extensions/verify-generated-output/nested/extra.txt"
        ] = "unexpected"

        with self.assertRaisesRegex(ValueError, "global catalog layout"):
            self._validate(document)

    def test_fixture_keys_reject_host_neutral_absolute_paths(self):
        paths = (
            r"C:\workspace\fixture.txt",
            "C:/workspace/fixture.txt",
            r"\\server\share\fixture.txt",
            r"\rooted\fixture.txt",
            "/var/tmp/fixture.txt",
            "~/fixture.txt",
            r"~\fixture.txt",
        )
        for path in paths:
            with self.subTest(path=path):
                document = copy.deepcopy(self.document)
                case = self._case(
                    document, "single-observation-remains-observed"
                )
                case["fixture"][path] = "content"
                with self.assertRaisesRegex(ValueError, "absolute.*path"):
                    self._validate(document)

    def test_fixture_values_reject_host_neutral_absolute_paths(self):
        paths = (
            r"C:\workspace\fixture.txt",
            "C:/workspace/fixture.txt",
            r"\\server\share\fixture.txt",
        ...
        for path in paths:
            with self.subTest(path=path):
                ...
                case["fixture"]["evidence/task-a.md"] = (
                    f"Read artifact at {path}"
                )
                with self.assertRaisesRegex(ValueError, "absolute.*path"):
                    self._validate(document)

    def test_fixture_values_reject_wrapped_absolute_paths(self):
        paths = (
            r"C:\workspace\fixture.txt",
            r"\\server\share\fixture.txt",
            r"\rooted\fixture.txt",
        ...
        wrappers = ("Read({path})", "[artifact]({path})")
        for path in paths:
            for wrapper in wrappers:
                value = wrapper.format(path=path)
                with self.subTest(value=value):
                    document = copy.deepcopy(self.document)
                    case = self._case(
                        document, "single-observation-remains-observed"
                    )
                    case["fixture"]["evidence/task-a.md"] = value
                    with self.assertRaisesRegex(ValueError, "absolute.*path"):
                        self._validate(document)

    def test_fixture_values_reject_assignment_and_backtick_path_bypasses(self):
        values = (
            r"[artifact](C:\workspace\fixture=name.txt)",
            "[artifact](/var/tmp/fixture=name.txt)",
            r"Read `C:\workspace\fixture.txt`",
            "Read `/var/tmp/fixture.txt`",
        )
        for value in values:
            with self.subTest(value=value):
                document = copy.deepcopy(self.document)
    ...
    def test_fixture_values_allow_non_absolute_slash_forms(self):
        values = (
            "Read(https://example.com/artifacts/fixture.txt)",
            "[artifact](https://example.com/artifacts/fixture.txt)",
            "[artifact](https://example.com/fixture.txt?name=/var/tmp/value)",
            "artifact=docs/fixture.txt",
            "Run /skills before continuing",
            "Preserve the 16/16 coverage ratio",
            "Read(docs/fixture.txt)",
            "[artifact](docs/fixture.txt)",
            "Read `docs/fixture.txt`",
        )
        for value in values:
    ...
    def test_evaluation_cases_must_be_objects_before_id_extraction(self):
        for invalid_case in ("scalar-case", ["list-case"]):
            with self.subTest(invalid_case=invalid_case):
                document = copy.deepcopy(self.document)
                document["cases"][0] = invalid_case
                with self.assertRaisesRegex(
                    ValueError, "Evaluation case must be an object"
                ):
                    self._validate(document)
...
class StrictInfrastructureTests(unittest.TestCase):
    ...
    def test_supported_semver_range(self):
        self.assertTrue(version_satisfies("2.4.1", ">=2.4.0 <3.0.0"))
        self.assertFalse(version_satisfies("3.0.0", ">=2.4.0 <3.0.0"))
    ...
    def test_unsupported_constraint_is_rejected_after_failed_comparison(self):
        with self.assertRaisesRegex(
            AdaptiveError, "invalid version constraint: \\^3.0.0"
        ):
            version_satisfies("1.0.0", ">=2.0.0 ^3.0.0")
...
class ExtensionValidationTests(unittest.TestCase):
    def test_valid_augment_extension_passes(self):
        ...
        metadata = validate_extension(extension, "2.4.0", core_registry())
    ...
    def test_validation_rejects_invalid_metadata_contracts(self):
        cases = (
            ...
            ("evidence", [], "evidence"),
            ("compatibleSdlc", ">=3.0.0", "compatible"),
        )
    ...
    def test_extension_id_must_match_directory_and_not_core(self):
        ...
        with self.assertRaisesRegex(AdaptiveError, "core module"):
            validate_extension(extension, "2.4.0", core_registry())
...
class PromotionTests(unittest.TestCase):
    ...
    def test_upstream_proposal_contains_substantive_review_information(self):
        ...
        self.assertIn(">=2.0.0 <4.0.0", proposal)
...
class DiscoveryTests(unittest.TestCase):
    ...
    def test_incompatible_extension_is_rejected(self):
        create_extension_at(
            self.root, id="future-only", compatibleSdlc=">=3.0.0"
        )
